...
async function getDashboardData() {
    ...
    // 5. Facturas Pendientes (nuevo)
    // @ts-ignore
    const { count: facturasPendientes } = await adminClient
        .from("facturas")
        .select("*", { count: "exact", head: true })
        .in("estado", ["pendiente", "parcial", "vencida"]);

    // 6. Ingresos del Mes (nuevo)
    const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
    // @ts-ignore
    const { data: pagosDelMes } = await adminClient
        .from("pagos")
        .select("monto")
        .gte("fecha", firstDayOfMonth)
        .lte("fecha", nowStr);

    const ingresosDelMes = (pagosDelMes || []).reduce((sum: number, p: any) => sum + (p.monto || 0), 0);

    // 7. Actividades Recientes (sin filtro de usuario para mostrar todas)
    // @ts-ignore
...
export default async function DashboardPage() {
    ...
    const cards = [
        ...
        },
        {
            ...
            value: `$${(stats.ingresos_mes || 0).toLocaleString("es-CO")}`,
            icon: DollarSign,
    ...
    const getActivityIcon = (type: string | null) => {
        switch (type) {
            ...
            case 'pago_registrado': return CreditCard;
            case 'factura_generada': return Receipt;
            case 'factura_enviada': return Send;
            case 'lead_convertido': return CheckCircle2;
            case 'cotizacion_enviada': return Send;
            case 'cita_programada': return Calendar;
    ...
    const getActivityColor = (type: string | null) => {
        switch (type) {
            case 'pago_registrado': return 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400';
            case 'factura_generada': return 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400';
            case 'factura_enviada': return 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400';
            case 'lead_convertido': return 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400';
            case 'cotizacion_enviada': return 'bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400';
            case 'cita_programada': return 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400';
            case 'llamada': return 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400';
                                    ...
                                    <div key={activity.id} className="flex items-start space-x-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg transition-colors">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                {formatColombiaDate(activity.fecha, 'short-datetime')}
                                            </p>
                                        </div>
                                    </div>
                                );
